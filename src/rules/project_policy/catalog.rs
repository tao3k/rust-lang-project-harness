...
pub(super) fn rules_by_id() -> BTreeMap<&'static str, RustHarnessRule> {
    [
        ...
        RustHarnessRule::new(
            ...
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "Legacy cargo-test target gate should migrate",
            "Root Cargo test target harness gates are compatibility-only; migrate parser-native project policy to the cargo-check build gate and keep root test targets as thin suite aggregates.",
            labels("project-policy"),
        ),
        RustHarnessRule::new(
            RUST_PROJ_R007,
        ...
        RustHarnessRule::new(
            ...
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "Legacy source cargo-test gate should migrate",
            "Source cargo-test harness gates are compatibility-only; migrate parser-native project policy to the cargo-check build gate and remove the source test gate once the build gate is active.",
            labels("project-policy"),
        ),
        RustHarnessRule::new(
            RUST_PROJ_R010,
        ...
        RustHarnessRule::new(
            ...
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "Legacy cargo-test advice allowance lacks explanation",
            "Legacy cargo-test advice allowance must include an explicit explanation so agents cannot silence advisory policy just to pass the test layer.",
            labels("project-policy"),
        ),
        RustHarnessRule::new(
            RUST_PROJ_R016,
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "Legacy cargo-test gate uses empty verification config",
            "Source cargo-test compatibility gates that remain in use must declare verification profile hints, task suppressions, or skill bindings for the test layer.",
            labels("project-policy"),
        ),
    ]
    .into_iter()
